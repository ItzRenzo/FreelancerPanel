...
public class PendingQuotesController {
    ...
    public void loadQuoteData() {
        ...
        // Updated query to filter for quotes with status 'Pending'
        String query = "SELECT q.quote_id, q.quote_title, c.client_name, q.quote_price, q.quote_start_date, " +
                ...
                "WHERE q.user_id = ? AND q.quote_status = ?"; // Added condition for "Pending" status

        try (Connection connection = DatabaseHandler.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(query)) {

            preparedStatement.setInt(1, userId);  // Set the user ID parameter
            preparedStatement.setString(2, "Pending");  // Set the quote status to "Pending"
    ...
    public void EditClicked(MouseEvent event) {
        try {
            ...
            idController.setPendingQuotesController(this); // Pass reference for further actions
    ...
    public void NewClicked(MouseEvent event) {
        try {
            ...
            newQuoteController.setPendingQuotesController(this);
